...
class CheckInViewApp extends CheckInCore {
    constructor() {
    ...
    populateTeamFilter() {
        ...
        // Get all unique teams from teams array
            ...
            if (event && event.matches) {
                ...
                if (match && match.cards && match.cards.length > 0) {
                    match.cards.forEach(card => {
                        ...
                        const team = this.teams.find(t => t.id === card.teamId);
                        const member = team?.members?.find(m => m.id === card.memberId);
                        
                        allCards.push({
                            ...card,
                            eventName: game.eventName,
                            eventDate: epochToPacificDate(game.eventDate_epoch),
                        ...
                        <div class="game-note-item">
                            <div class="note-header">
                                <div class="note-game-info">
                                    ...
                                    <div class="note-details">${epochToPacificDate(game.eventDate_epoch)} • ${game.eventName} • ${game.field ? `Field ${game.field}` : 'No field'}</div>
                                </div>
            ...
            .map(member => {
                ...
                return `
                    <div class="player-grid-item ${isCheckedIn ? 'checked-in' : ''} ${isLocked ? 'locked' : ''} ${isSuspended ? 'suspended' : ''} ${cardsDisplay ? 'has-cards' : ''}"
                        ...
                        <div class="grid-check-icon">${isSuspended ? '🚫' : ''}</div>
    ...
    confirmForfeit(eventId, matchId) {
        ...
        // Store the selected team BEFORE closing dialog (which clears selectedForfeitTeam)
        const selectedTeam = this.selectedForfeitTeam;
        
        // Close dialog first
        this.closeForfeitDialog();
        
        // Set forfeit state and scores using our stored value
        this.isForfeitMatch = true;
        this.forfeitingTeam = selectedTeam;  // Use stored value, not this.selectedForfeitTeam
